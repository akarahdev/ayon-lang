...
public record Store(Expression lhs, Expression rhs, Mutable<Type> type, SpanData errorSpan) implements Expression {
    ...
    public Value llvm(CodeBlock codeBlock, boolean dereferenceLocals, FunctionTransformer transformer) {
        ...
        if(this.rhs().type().get() instanceof Type.UserStructure) {
            ...
            transformer.basicBlocks.peek().ifThenElse(
                ...
                ifTrue -> {
                    ReferenceCountingLibrary.debugPrint(ifTrue, transformer.module, "true!");
                    ifTrue.br(outputBlock.name());
                },
                ifFalse -> {
                    ReferenceCountingLibrary.debugPrint(ifFalse, transformer.module, "false!");
                    ifFalse.call(
                        Types.integer(16),
                        ReferenceCountingLibrary.DECREMENT_REFERENCE_COUNT,
                        List.of(new Call.Parameter(Types.pointer(), ref))
                    );
